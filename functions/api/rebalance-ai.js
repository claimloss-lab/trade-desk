...
กฎ: stocks ครบทุกตัว, targetPct รวม = 100 พอดี, เหตุผลภาษาไทยสั้นกระชับ, ห้าม disclaimer`;
    ...
    // web search อาจทำให้ API ตอบ pause_turn (พักกลางคัน) — ต้องวนส่งต่อให้คิดจนจบ
    ...
    // ดึง JSON ก้อนสุดท้ายแบบนับวงเล็บถ่วงดุล (ทน preamble ที่มี { ปน)
    function extractJson(t) {
      ...
      for (let i = end; i >= 0; i--) {
        if (t[i] === '}') depth++;
          ...
          depth--;
          if (depth === 0) {
            try { return JSON.parse(t.slice(i, end + 1)); } catch { /* scan ต่อ */ }
          }
        }
      }
      return null;
